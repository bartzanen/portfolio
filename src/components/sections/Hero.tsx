...
export function Hero({ data }: { data: HeroData }) {
  ...
  return (
    <section id="top" className="mx-auto flex min-h-[88svh] w-full max-w-content flex-col justify-center px-5 pb-12 pt-28 sm:px-8">
      {data.eyebrow && (
        <motion.p {...enter(0)} className="font-mono text-sm text-teal-700 dark:text-teal-400">
          {data.eyebrow}
        </motion.p>
      )}
      ...
      {data.tagline && (
        <motion.p {...enter(0.24)} className="mt-5 max-w-xl text-base leading-relaxed sm:text-lg">
          {data.tagline}
        </motion.p>
      )}
